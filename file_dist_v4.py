...
class FileTransfer:
    def _init_(self, file_path, group_name, admin):
        ...
        self.transfer_id = hashlib.md5(f"{group_name}{file_path}{time.time()}".encode()).hexdigest()

    def start(self, fernet):
        ...
        try:
            with open(self.file_path, 'rb') as file:
                ...
                while chunk := file.read(CHUNK_SIZE):
                    file_content += chunk
                    self.progress = (len(file_content) / self.total_size) * 100
                ...
                encrypted_content = fernet.encrypt(compressed_content)
...
class FileDistributionServer:
    ...
    def distribute_files(self, group_name, file_paths, admin, silent=False, schedule_time=None):
        ...
        if schedule_time:
            job_id = hashlib.md5(f"{group_name}{file_paths}{schedule_time}".encode()).hexdigest()
            self.scheduled_transfers[job_id] = {
                'group_name': group_name,
                'file_paths': file_paths,
                'admin': admin,
                'silent': silent
            }
            schedule.every().day.at(schedule_time).do(self._execute_scheduled_transfer, job_id)
            return {'status': 'success', 'message': f'File distribution scheduled for {schedule_time}', 'job_id': job_id}
        else:
            return self._distribute_files(group_name, file_paths, admin, silent)

    def _execute_scheduled_transfer(self, job_id):
        job = self.scheduled_transfers.pop(job_id, None)
        if job:
            return self._distribute_files(**job)
        return {'status': 'error', 'message': 'Scheduled job not found'}

    def _distribute_files(self, group_name, file_paths, admin, silent):
        successful_files = []
        failed_files = []
        ...
        for file_path in file_paths:
            ...
            transfer = FileTransfer(file_path, group_name, admin)
            self.transfers[transfer.transfer_id] = transfer
            
            result = transfer.start(self.fernet)
            if result:
                with self.lock:
                    self.groups[group_name]['files'].append(result)
        ...
        return {
            'status': 'success',
            'message': 'File distribution completed',
    ...
    def pause_transfer(self, transfer_id, admin):
        ...
        if transfer_id in self.transfers:
            self.transfers[transfer_id].pause()
    ...
    def resume_transfer(self, transfer_id, admin):
        ...
        if transfer_id in self.transfers:
            self.transfers[transfer_id].resume()
    ...
    def transfer_progress(self, transfer_id):
        if transfer_id in self.transfers:
            ...
            return {'status': 'success', 'state': transfer.state, 'progress': transfer.progress}
        return {'status': 'error', 'message': 'Transfer not found'}
